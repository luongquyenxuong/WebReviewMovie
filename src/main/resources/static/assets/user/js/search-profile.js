...
function searchPostByConditionTime(page) {
    ...
    apiSearchProfile(getCondition(), page);

}


function apiSearchProfile(condition, page) {
    ...
    call(`http://localhost:8080/api/search-post-profile?page=${page - 1}`, 'post', condition)
        .then(response => {
    ...
    for (const item of postList) {
        result += `
                                        ...
                                        <div class="card col-4 p-0" >
                                          <form method="post" action="/post/edit-post?postId=${item.id}">
                                                <button><i class="mdi mdi-tooltip-edit"></i></button>
                                            </form>
                                             ...
                                             <img class="card-img-bottom" src="${'../uploads/' + item.imagePost}" style="width:100%;height:300px ;object-fit: cover" alt="">
                                                <div class="card-body">
                                                 <h4 class="card-title text-dark">${item.title}</h4>
                                                  <p class="card-text">${'Trạng thái: ' + item.status}</p>
